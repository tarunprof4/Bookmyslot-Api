...
namespace Bookmyslot.Api.Customers.Repositories.Queries
{
    ...
    public class CustomerTableQueries
    {
        ...
        public const string RegisterCustomerQuery = @"INSERT INTO" + " " + DatabaseConstants.RegisterCustomerTable + " " + @"(Id, FirstName, LastName, UserName, Email, Provider, PhotoUrl, CreatedDateUtc)
VALUES(@Id, @FirstName, @LastName, @UserName, @Email, @Provider, @PhotoUrl, @CreatedDateUtc); ";
 ...
 WHERE Id=@customerId";
        ...
        public const string GetCustomersByCustomerIdsQuery = @"select Id, Email, FirstName, LastName, BioHeadLine from" + " " + DatabaseConstants.RegisterCustomerTable + " " + "where Id IN @CustomerIds";

        public const string GetCustomerByEmailQuery = @"select Id, Email, FirstName, LastName, BioHeadLine from" + " " + DatabaseConstants.RegisterCustomerTable + " " + "where Email = @Email";

        public const string GetCustomerIdByEmailQuery = @"select Id from" + " " + DatabaseConstants.RegisterCustomerTable + " " + "where Email = @Email";

        public const string GetCustomerByIdQuery = @"select Id, Email, FirstName, LastName, BioHeadLine from" + " " + DatabaseConstants.RegisterCustomerTable + " " + "where Id = @CustomerId";
